...
void Slider::Draw() const {
    // OK TODO HACKATHON-5 (3/4): The slider's component should be drawn here.
    Bar.Draw();
    End1.Draw();
    End2.Draw();
    ImageButton::Image::Draw();
}
void Slider::SetOnValueChangedCallback(std::function<void(float value)> onValueChangedCallback) {
    OnValueChangedCallback = onValueChangedCallback;
}
void Slider::SetValue(float value) {
    // OK TODO HACKATHON-5 (4/4): Set the value of the slider and call the callback.
    this->value = value;
    Position.x = End1.Position.x + Bar.Size.x * value;
